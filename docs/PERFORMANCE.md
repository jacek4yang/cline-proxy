...
| Cline HTTP client | one process-wide `reqwest::Client` | rustls + HTTP/2 + gzip; `tcp_nodelay`; direct mode `no_proxy()` so env proxies cannot leak onto Cline; optional SOCKS5/SOCKS5h for Cline only |
